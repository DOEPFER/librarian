...
step_0 = Step(
    name='Index',
    description="Update the library's index files",
    executor=update_index
)
...
step_3 = Step(
    ...
    description='Check the semantic similarity with other files and, if a match is found, move it to the same folder (shelf).',
...
dag = Workflow(
    ...
    steps=[
        ...
        step_3,
        Condition(
            name='Semantic-similarity-not-found',
            evaluator=lambda step_input: step_input.get_step_output(step_name='Semantic-similarity').success == False,
            steps=[step_4, step_5]
        ),
        step_6
    ],
